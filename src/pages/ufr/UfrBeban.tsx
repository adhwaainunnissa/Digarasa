...
import type { UfrBebanData, PaginationMeta } from "../../services/api/ufrService";
...
export default function UfrBeban() {
    ...
    const loadData = async (pageToLoad: number = pagination.page) => {
        setLoading(true);
        try {
            const result = await getUfrBeban(beban, pageToLoad, pagination.limit, search);
            setData(result.data);
            ...
            <header className="shrink-0 border-b border-slate-200 bg-white px-8 py-6">
                <div className="mx-auto flex max-w-7xl items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">UFR Beban</h1>
                            <p className="mt-1 text-sm font-medium text-slate-500">Pemantauan Beban UFR - UP2D (Read Only)</p>
                        </div>
                    </div>
                    <button 
            ...
            <main className="flex-1 overflow-hidden flex flex-col mx-auto w-full max-w-7xl p-8">
                ...
                <div className="flex items-center justify-between mb-6 shrink-0">
                    ...
                    <div className="relative w-72">
                        ...
                        <input
                            type="text"
                            className="block w-full rounded-md border-0 py-2 pl-10 pr-3 text-slate-900 ring-1 ring-inset ring-slate-300 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 shadow-sm"
                                    ...
                                    <tr>
                                        <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                                            <div className="flex flex-col items-center justify-center">
                                                ...
                                                <span className="font-medium">Memuat data UFR Beban {beban}...</span>
                                            </div>
                                        </td>
                                    </tr>
                                ) : data.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                                            <div className="flex flex-col items-center justify-center">
                                                ...
                                                <span className="text-sm font-medium">Tidak ada data ditemukan untuk Beban {beban}</span>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                    data.map((row, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className="font-mono text-slate-700 bg-slate-100 px-2 py-1 rounded">{row.value ?? "-"}</span>
                                            </td>
                                            <td className="px-6 py-4">
